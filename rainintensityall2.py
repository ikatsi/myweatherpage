...
Single entrypoint for all rain-intensity maps:
  - Greece (WGS84 degrees) + snowflakes + MTG-LI lightning + snowline altitude labels (local lapse regression)
  - Attica / Crete / NE Greece / SW Greece (EGSA87 EPSG:2100 meters) + snowflakes + MTG-LI lightning + snowline altitude labels
...
# EUMETSAT MTG Lightning Imager (LI-2-LFL) controls.
# The credentials are already expected to be GitHub Actions secrets.
...
MTG_LI_COLLECTION_ID = "EO:EUM:DAT:0691"
LIGHTNING_WINDOW_MIN = int(os.environ.get("LIGHTNING_WINDOW_MIN", "15"))
LIGHTNING_QUERY_LOOKBACK_MIN = int(
    os.environ.get("LIGHTNING_QUERY_LOOKBACK_MIN", str(LIGHTNING_WINDOW_MIN))
)
LIGHTNING_CACHE_MAX_AGE_MIN = int(os.environ.get("LIGHTNING_CACHE_MAX_AGE_MIN", "5"))
LIGHTNING_MIN_SEP_DEG = float(os.environ.get("LIGHTNING_MIN_SEP_DEG", "0.12"))
LIGHTNING_MAX_ICONS = int(os.environ.get("LIGHTNING_MAX_ICONS", "500"))
LIGHTNING_FONTSIZE = float(os.environ.get("LIGHTNING_FONTSIZE", "10"))

# Exactly the same rectangular grid used by run_greece().  LI products are
# full-disc files, so the geographic subset is applied immediately after each
# file is read and only these Greece points are retained in memory/cache.
LIGHTNING_LON_MIN, LIGHTNING_LON_MAX = 19.0, 30.0
LIGHTNING_LAT_MIN, LIGHTNING_LAT_MAX = 34.5, 42.5
_LIGHTNING_POINTS_MEMORY = None
...
# =============================================================================
# EUMETSAT MTG LIGHTNING IMAGER HELPERS (Greece grid only)
# =============================================================================
def _read_lightning_cache(cache_path: str, max_age_minutes: int):
    try:
        ...
        age = datetime.now(ZoneInfo("UTC")) - created
        if age > timedelta(minutes=max_age_minutes):
            return None
        return [(float(p[0]), float(p[1])) for p in payload.get("points", [])]
    except Exception:
        return None


def _extract_li_flashes_from_netcdf_bytes(raw: bytes):
    """Extract valid lon/lat flash centroids from one LI-2-LFL NetCDF body."""
    from netCDF4 import Dataset

    points = []
    ds = Dataset("mtg_li_lfl.nc", mode="r", memory=raw)
    try:
        ...
        lons = np.ma.asarray(lon_var[:]).filled(np.nan).astype(float).ravel()
        lats = np.ma.asarray(lat_var[:]).filled(np.nan).astype(float).ravel()

        valid = (
            np.isfinite(lons) & np.isfinite(lats) &
            (lons >= LIGHTNING_LON_MIN) & (lons <= LIGHTNING_LON_MAX) &
            (lats >= LIGHTNING_LAT_MIN) & (lats <= LIGHTNING_LAT_MAX)
        )
        points.extend(zip(lons[valid].tolist(), lats[valid].tolist()))
    finally:
        ds.close()
    return points


def _extract_li_product(product):
    """Read the NetCDF BODY entries of one EUMDAC LI product."""
    points = []
    entries = list(product.entries)
    nc_entries = [
        entry for entry in entries
        if str(entry).lower().endswith(".nc") and "BODY" in str(entry).upper()
    ]
    # Some archived products do not include BODY in the component name.
    if not nc_entries:
        nc_entries = [entry for entry in entries if str(entry).lower().endswith(".nc")]

    for entry in nc_entries:
        try:
            with product.open(entry=entry) as source:
                points.extend(_extract_li_flashes_from_netcdf_bytes(source.read()))
        except Exception as e:
            print(f"⚠️ Could not read MTG-LI entry {entry}: {e}")
    return points


def _thin_lightning_points(points, min_sep_deg=LIGHTNING_MIN_SEP_DEG,
                           max_points=LIGHTNING_MAX_ICONS):
    """Keep recent-flash coverage legible by placing at most one icon nearby."""
    if not points:
        return []

    # Rounded-coordinate de-duplication also protects against repeated chunks.
    unique = list(dict.fromkeys((round(lon, 4), round(lat, 4)) for lon, lat in points))
    kept = []
    min_sep2 = float(min_sep_deg) ** 2
    for lon, lat in unique:
        if all((lon - x) ** 2 + (lat - y) ** 2 >= min_sep2 for x, y in kept):
            kept.append((lon, lat))
            if len(kept) >= int(max_points):
                break
    return kept


def fetch_mtg_li_greece_flashes():
    """
    Download recent LI-2-LFL products once and retain only Greece-grid flashes.

    Failure is intentionally non-fatal: rain/snow maps continue unchanged. A
    cache no older than the lightning display window may be used as fallback.
    """
    global _LIGHTNING_POINTS_MEMORY
    if _LIGHTNING_POINTS_MEMORY is not None:
        return _LIGHTNING_POINTS_MEMORY

    cache_path = os.path.join(BASE_DIR, "mtg_li_greece_cache.json")
    if not EUMETSAT_CONSUMER_KEY or not EUMETSAT_CONSUMER_SECRET:
        print("⚠️ EUMETSAT credentials missing; continuing without lightning.")
        _LIGHTNING_POINTS_MEMORY = []
        return []

    now_utc = datetime.now(ZoneInfo("UTC"))
    cached = _read_lightning_cache(cache_path, LIGHTNING_CACHE_MAX_AGE_MIN)

    try:
        import eumdac
        # Import here so a missing optional package cannot stop rain/snow maps.
        from netCDF4 import Dataset as _NetCDF4Dataset  # noqa: F401

        credentials = (EUMETSAT_CONSUMER_KEY, EUMETSAT_CONSUMER_SECRET)
        token = eumdac.AccessToken(credentials)
        datastore = eumdac.DataStore(token)
        collection = datastore.get_collection(MTG_LI_COLLECTION_ID)

        # EUMDAC examples use naive UTC datetime values for sensing-time search.
        search_end = now_utc.replace(tzinfo=None)
        search_start = (
            now_utc - timedelta(minutes=LIGHTNING_QUERY_LOOKBACK_MIN)
        ).replace(tzinfo=None)
        products = list(collection.search(dtstart=search_start, dtend=search_end))
        print(f"⚡ MTG-LI products found: {len(products)}")

        all_points = []
        for product in products:
            try:
                all_points.extend(_extract_li_product(product))
            except Exception as e:
                print(f"⚠️ Could not process MTG-LI product {product}: {e}")

        points = _thin_lightning_points(all_points)
        payload = {
            "created_at": now_utc.isoformat(),
            "window_minutes": LIGHTNING_WINDOW_MIN,
            "bbox": [
                LIGHTNING_LON_MIN, LIGHTNING_LON_MAX,
                LIGHTNING_LAT_MIN, LIGHTNING_LAT_MAX,
            ],
            "points": points,
        }
        ...
        print(
            f"⚡ MTG-LI flashes in Greece grid: {len(all_points)}; "
            f"icons after thinning: {len(points)}"
        )
        _LIGHTNING_POINTS_MEMORY = points
        return points
    except Exception as e:
        if cached is not None:
            print(f"⚠️ MTG-LI retrieval failed ({e}); using recent cached lightning.")
            _LIGHTNING_POINTS_MEMORY = cached
            return cached
        print(f"⚠️ MTG-LI retrieval failed ({e}); continuing without lightning.")
        _LIGHTNING_POINTS_MEMORY = []
        return []


def plot_mtg_li_icons(ax, bbox, transform_xy=None):
    """Plot cached Greece-grid flashes within a map bbox."""
    lon_min, lon_max, lat_min, lat_max = bbox
    points = [
        (lon, lat) for lon, lat in fetch_mtg_li_greece_flashes()
        if lon_min <= lon <= lon_max and lat_min <= lat <= lat_max
    ]
    for lon, lat in points:
        x, y = transform_xy(lon, lat) if transform_xy else (lon, lat)
        ax.text(
            x, y, "⚡",
            ha="center", va="center",
            fontsize=LIGHTNING_FONTSIZE,
            color="#ffd000",
            zorder=8,
            path_effects=[pe.withStroke(linewidth=1.5, foreground="black")],
        )
    print(f"⚡ MTG-LI icons placed on {bbox}: {len(points)}")
    return len(points)
...
def run_egsa_region(cfg: dict):
    ...
    plot_mtg_li_icons(
        ax,
        (lon_min, lon_max, lat_min, lat_max),
        transform_xy=lambda lon, lat: wgs_to_egsa.transform(lon, lat),
    )
